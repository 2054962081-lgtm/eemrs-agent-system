...
| Milvus 医疗知识库 | ✅ | 支持知识校验、切片、向量化、入库、重建与检索 |
| RAG 接入预问诊 | ✅ | 检索结果进入 QuestionPlan 与 Prompt，支持快速/深度问诊场景 |
| 本地 Embedding | ✅ | 使用 `BAAI/bge-small-zh-v1.5`，不通过云端大模型生成向量 |
| QuestionPlan / Must-Ask | ✅ | 将检索知识转换为结构化追问，并对遗漏必问项进行后处理补齐 |
| RAG A/B 评测 | ✅ | 已提供批量对比脚本，用于评估启用/关闭 RAG 的差异 |
| Trace Harness 与 Bad Case | 🟡 | 已有运行轨迹和评测基础，仍需完善指标看板与持续回归 |
...
| 报告密文读取与本地解密 | ✅ | Java 侧在本地读取并解密历史报告，原始敏感内容不直接交给云端模型 |
| 指标结构化、脱敏与趋势计算 | ✅ | 统一指标名称、单位和时间，提取异常项及跨报告变化，形成结构化趋势数据 |
| 报告纵向分析 MVP | ✅ | 基于多次报告生成趋势摘要、变化解释和复查提示，云端仅接收脱敏后的最小必要趋势 JSON |
| 纵向分析与预问诊上下文融合 | 🟡 | 核心能力已完成，正在进一步融合当前症状、既往病史和问诊上下文 |
| 报告趋势可视化与医生协同 | 🟡 | 已具备分析结果，仍需继续完善患者端/医生端图表展示和审核反馈闭环 |
...
| 报告智能分析 | **纵向分析 MVP 已完成** | 本地解密、脱敏、指标结构化、趋势计算和模型解读主链路已形成，继续完善上下文融合与前端展示 |
| 工程部署 | **开发环境阶段** | 需补 Docker Compose、CI/CD、监控、灰度和配置中心 |

**总体定位：一个具备真实医疗业务、Milvus RAG、结构化问诊规划、报告纵向分析和评测 Harness 的医疗 AI 全栈 MVP。
